window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = ".dshdpRow{box-sizing:border-box;margin:6px 0 6px 4px;color:var(--dsw-alias-label-primary);flex-direction:column;display:flex;min-width:0}"
			...
			+ ".dshdpInspect{border:.5px solid var(--dsw-alias-border-l3);corner-shape:round;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-secondary);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;font-size:11px;line-height:16px;display:inline-flex;flex:none}"
		...
		const UNSAFE_CSS = [
			...
			"--diffs-bg-addition-emphasis-override:light-dark(color-mix(in srgb,var(--dsw-alias-state-success-primary) 14%,transparent),color-mix(in srgb,var(--dsw-alias-state-success-primary) 18%,transparent))!important;",
			"--diffs-bg-deletion-emphasis-override:light-dark(color-mix(in srgb,var(--dsw-alias-state-error-primary) 14%,transparent),color-mix(in srgb,var(--dsw-alias-state-error-primary) 18%,transparent))!important;",
